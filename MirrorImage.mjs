...
function renderButton(numImages, isActive) {
    return `<div id="mirrorImages${numImages}" class="control-icon${isActive ? " active": ''}" style="margin-top:0px;overflow:hidden;" data-tooltip="${numImages} image${numImages === 1 ? '' : 's'}">
    <img src="icons/magic/defensive/illusion-evasion-echo-purple.webp" style="margin:0;filter:grayscale(${(1 - (numImages / 3)) * 100}%);"/>
</div>`
}
...
export class MirrorImage {
    ...
    async renderHud(token, document) {
